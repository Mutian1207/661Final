...
def FGM_L2_attack(model, device, dat, lbl, eps):
    ...
    gradient_l2 = torch.clamp(gradient_l2, min=1e-12)
    gradient_l2 = gradient / torch.norm(gradient_l2, keepdim = True)
    # Add perturbation the data
    x_adv = x_nat + eps * torch.sign(gradient_l2)
    # Clip the perturbed datapoints to ensure we are in bounds [0,1]
    x_adv = torch.clamp(x_adv, min=0., max=1.)
